...
Vec3 hittable_material_color(const Ray& r, Hittable *world, int depth) {
    hit_record rec;
...
TEST(QAmetal, metal)
{
    ...
    CBmpEncoder bmpEncoder("metal.bmp", rawBmp.data(), nx, ny, false);
    bmpEncoder.Write();
}

TEST(QAmetal, dialectric)
{
    int nx = 1000;
    int ny = 500;
    int ns = 100;
    ...
    CBmpEncoder bmpEncoder("dialectric.bmp", rawBmp.data(), nx, ny, false);
    bmpEncoder.Write();
}
